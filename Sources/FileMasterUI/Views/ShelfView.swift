...
struct ShelfView: View {
    ...
    /// Active citation in the source pane plus every sibling source from the
    /// same turn, so the pane can render the numbered strip and ◀ ▶ navigation.
    @State private var viewingCitation: Citation? = nil
    @State private var viewingSiblings: [Citation] = []
    ...
    @State private var currentPreviewURL: URL? = nil
    ...
    private var currentSize: CGSize {
        ...
        if isPreviewing { return previewSize }
    ...
    var body: some View {
        ...
        .animation(.spring(response: 0.38, dampingFraction: 0.82), value: isPreviewing)
        ...
        .onChange(of: isPreviewing) { _, previewing in
            if !previewing { currentPreviewURL = nil }
            postResize()
            // Make the den key so the preview's scroll/keyboard input lands here.
            onAskModeChanged?(previewing)
        }
        ...
        VStack(spacing: 0) {
                            ...
                            ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
                                ExpandedItemView(
                                    ...
                                    isEditTab: (isEditing && currentEditURL == item.url)
                                        || (isPreviewing && currentPreviewURL == item.url),
            ...
            HStack {
                ...
                ActionsMenuButton(
                    ...
                    onPreview: { urls in enterPreview(urls: urls) }
                )
    ...
    private var askSplitView: some View {
        HStack(spacing: 0) {
            ...
            if let citation = viewingCitation {
                ...
                Divider().opacity(0.4)
                CitationPane(citations: viewingSiblings.isEmpty ? [citation] : viewingSiblings,
                             active: citation,
                             onSelect: { viewingCitation = $0 },
                             onClose: { withAnimation { viewingCitation = nil; viewingSiblings = [] } })
                    .frame(maxWidth: .infinity)
            } else {
                askPane
                    .frame(maxWidth: .infinity)
        ...
        VStack(spacing: 0) {
            ...
            if let askSession {
                QAView(session: askSession,
                       onOpenCitation: { citation, siblings in
                           withAnimation {
                               viewingSiblings = siblings
                               viewingCitation = citation
                           }
                       })
            } else {
                Spacer()
    ...
    // MARK: - Preview split (files left, embedded Quick Look right)

    private func previewSplitView(url: URL) -> some View {
        HStack(spacing: 0) {
            expandedView
                .frame(width: expandedSize.width)
            Divider().opacity(0.4)
            // `.id(url)` forces a fresh QLPreviewView per tab so the preview
            // resets cleanly when switching files.
            EmbeddedPreviewView(
                url: url,
                onClose: { withAnimation { isPreviewing = false } },
    ...
    /// Open the embedded preview beside the file view, or switch to it if
    /// already previewing — the file list acts as preview tabs (mirroring the
    /// editor flow). Multiple URLs from a menu selection just preview the first;
    /// the rest are reachable by clicking their tile.
    private func enterPreview(urls: [URL]) {
    ...
    /// the active editor tab; while previewing, any file becomes the active
    ...
    private func removeURLs(_ urls: [URL]) {
        ...
        // Same for the preview tab — any file works there, not just images.
        if let current = currentPreviewURL, set.contains(current) {
            if let next = items.first?.url {
                enterPreview(urls: [next])
            } else {
    ...
    private func shareAll(from view: NSView) {
        ...
        if hasDirectory {
            if FileMasterSettings.shared.autoZipOnShare {
                shareAsZip()
            } else {
    ...
    private func shareAsZip() {
        ...
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("FileMaster-\(Int(Date().timeIntervalSince1970)).zip")
    ...
    /// Space bar → embedded preview for the current selection, mirroring Finder
    /// but rendered inline in the den. Scoped to this den's window, and
    /// suppressed while typing (e.g. the Ask field).
    private func startKeyMonitor() {
        keyMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown]) { event in
            ...
            // Space while the system Quick Look panel is focused → close it
            // (legacy paths can still open it; mirror the original toggle).
            ...
            // Space while previewing → close the embedded preview (toggle).
            if isPreviewing {
                withAnimation { isPreviewing = false }
                return nil
            }
